...
BOOL CServerSocketApp::InitInstance()
{
	...
	CClientSocketDlg dlg3;
	switch( m_nLinkMode )
	{
		default:
		case 0:
			m_pMainWnd = &dlg1; // Gateway
			break;
		case 1:
			m_pMainWnd = &dlg2; // Server
			break;
		case 2:
			m_pMainWnd = &dlg3; // Client
			break;
	}
...
void CServerSocketApp::ParseCommandLineArgs()
{
	...
	if (!strCmdLine.IsEmpty())
	{
		...
		while( !strCmdLine.IsEmpty() )
		{
			...
			if (strCurrent == _T("/GATEWAY") || strCurrent == _T("/G") )
				m_nLinkMode	= 0;
			else if (strCurrent == _T("/SERVER") || strCurrent == _T("/S") )
				m_nLinkMode	= 1;
